    ...
    def train_step(self):
        ...
        for _ in range(self.grad_accum_every):
            ...
            while non_empty_batch is False:
                ...
                loss, all_logits, all_labels = self.train_wrapper(**data_kwargs, return_loss=True)
...
class ClapRVQTrainer(nn.Module):
    ...
    def train_step(self):
        ...
        for _ in tqdm(range(iters), desc='accumulating batches'):
            ...
            #idx, file, raw_wave_for_clap = batch[0], batch[1], batch[2][0]
...
@beartype_jit
class ClapRVQMultimodalTrainer(ClapRVQTrainer):
    def __init__(
        self,
        *,
        num_train_steps,
        ...
        from scripts.myscripts.omlm.multimodal_clap_rvq import SoundDatasetRVQ


        super(ClapRVQTrainer, self).__init__()
        self.accelerator = Accelerator(**accelerate_kwargs)
        ...
        if continue_from_step:
            current_step = continue_from_step
        else: current_step = 0
        self.register_buffer('steps', torch.Tensor([current_step]))

        if not exists(self.ds):
            assert exists(
                folders[0]), 'folder must be passed in, if not passing in a custom dataset for text conditioned audio synthesis training'

            self.ds = SoundDatasetRVQ(
                folders,
                max_length_seconds=data_max_length_seconds,
                target_sample_hz=audio_conditioner.sample_rate,
                seq_len_multiple_of=None,
                ignore_files=default(ignore_files, []),
                ignore_load_errors=ignore_load_errors,
                data_csv="./scripts/myscripts/shockwave/packaged/info.csv"
            )
        ...
        # dataloader
        import scripts.myscripts.omlm.multimodal_clap_rvq as multimodal_clap_rvq
        collate_fn = multimodal_clap_rvq.pad_to_longest_fn
        self.dl =  DataLoader(self.ds, collate_fn=collate_fn, batch_size=batch_size, shuffle=True)
        self.valid_dl = DataLoader(self.valid_ds, collate_fn=collate_fn, batch_size=batch_size, shuffle=True)
    ...
    def train_step(self):
        ...
        for _ in tqdm(range(iters), desc='accumulating batches'):
            #raw_wave_for_clap = next(self.dl_iter)[0]
            batch = next(self.dl_iter)
            audio_idx, audio_file, raw_wave_for_clap, text_idx, text_file, song_descs = batch[0], batch[1], batch[2][0], batch[3], batch[4], batch[5][0]
            if len(raw_wave_for_clap) > 0:
                embed = self.audio_conditioner.forward(audio_input=raw_wave_for_clap.to(self.device), return_embedding=True)
                embeds.append(embed)
            if len(song_descs) > 0:
                embed = self.audio_conditioner.forward(text_input=song_descs, return_embedding=True)
                embeds.append(embed)
        ...
        if self.is_main:
            ...
            # sample results every so often
            valid_loss = None
            valid_loss_audio = None
            valid_loss_text = None
            if not (steps % self.save_results_every):
                #raw_wave_for_clap = next(self.valid_dl_iter)[0]
                batch = next(self.valid_dl_iter)
                #idx, file, raw_wave_for_clap = batch[0], batch[1], batch[2][0]
                ...
                with torch.no_grad():
                    self.audio_conditioner.learn_rvq = False
                    if len(raw_wave_for_clap) > 0:
                        valid_loss_audio = self.audio_conditioner.forward(audio_input=raw_wave_for_clap.to(self.device), return_rvq_loss=True)
                    else: valid_loss_audio = 0.
                    if len(song_descs) > 0:
                        valid_loss_text = self.audio_conditioner.forward(text_input=song_descs, return_embedding=False, return_rvq_loss=True)
                    else: valid_loss_text = 0.
                    valid_loss = ((valid_loss_audio * len(raw_wave_for_clap)) + (valid_loss_text * len(song_descs))) / (len(raw_wave_for_clap) + len(song_descs))
...
class HfHubertKmeansTrainer(nn.Module):
    ...
    def extract_hubert_features(self):
        ...
        #batch = next(self.dl_iter)
        #idx, file, raw_wave = batch[0], batch[1], batch[2][0]
